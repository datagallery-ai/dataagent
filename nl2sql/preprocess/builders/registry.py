...
def build_step_builders(context: Step1BuildContext, only_steps: Iterable[str] | None = None) -> List[StepBuilder]:
    selected = {str(item).strip() for item in (only_steps or []) if str(item).strip()}
    builders: List[StepBuilder] = []
    for builder_cls in _STEP_BUILDERS:
        if selected and builder_cls.step_name not in selected:
            continue
        builders.append(builder_cls(context))
    return builders
