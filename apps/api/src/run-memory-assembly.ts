...
type CreateMetadataRunMemoryAssemblyInput = {
  isResume: boolean;
...
/** Metadata-only conversation assembly used by the Deep Agents runtime path. */
export const createMetadataRunMemoryAssembly = (
  input: CreateMetadataRunMemoryAssemblyInput
): RunMemoryAssembly => {
  const conversationMemory = new ConversationMemoryService({
    compactMemorySource: "metadata-summary",
  ...
  if (!input.isResume) {
    const currentUserRecord = conversationMemory.persistCurrentUserMessage({
