...
def main() -> None:
    ...
    # ap.add_argument("--db", required=True, help="Absolute path to sqlite database")
    ...
    ap.add_argument(
        "--inhibitor_concentration", default=1, help="Numeric concentration (will be stored as INTEGER/REAL)"
    )
    ...
    try:
        ...
        cur.execute(
            f"insert into {quote_ident(TABLE_EXPERIMENTS)} (id, start_date, submitter, operator, status, type) values ({args_id}, date(), 900001, NULL, 'NEW', 'neutralization')"
        )
