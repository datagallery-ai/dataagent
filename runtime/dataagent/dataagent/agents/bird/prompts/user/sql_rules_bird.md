...
7. Return exactly one read-only SELECT statement. Never return multiple SQL statements separated by semicolons. If the question mentions multiple aggregates, express the requested final row shape in one SELECT; use GROUP BY or UNION only when needed.
