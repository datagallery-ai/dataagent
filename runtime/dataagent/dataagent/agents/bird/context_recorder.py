...
_CURRENT_RECORDER: ContextVar[Optional[BirdContextRecorder]] = ContextVar("current_bird_context_recorder", default=None)


class BirdContextRecorder:
    """Record the current BIRD run in the shared Context trajectory DAG."""
    ...
    def record_action_hook(
        ...
        *,
        ...
        """Record one successfully completed BIRD workflow node."""
        _ = runtime
    ...
    @staticmethod
    def reset(token: Token[Optional[BirdContextRecorder]]) -> None:
    ...
    def create(
        ...
        *,
        ...
        try:
            ...
            logger.warning(f"Failed to initialize BIRD Context recorder: {exc}")
            return None

    def bind(self) -> Token[Optional[BirdContextRecorder]]:
    ...
    def record_action(self, *, node_name: str, state: Mapping[str, Any]) -> None:
        """Append one minimal Action node without changing BIRD state."""
        self._latest_state = state
        try:
            self._context.register_node(
                node_type="Action",
                description=f"BIRD node: {node_name}",
            ...
            logger.warning(f"Failed to record BIRD Context action {node_name}: {exc}")
    ...
    def finish(self, *, final_state: Optional[Mapping[str, Any]], completed: bool) -> None:
        ...
        try:
            ...
            logger.warning(f"Failed to persist BIRD Context trajectory: {exc}")
    ...
    def _register_response(self, state: Mapping[str, Any]) -> None:
        try:
            ...
            logger.warning(f"Failed to record BIRD Context response: {exc}")
