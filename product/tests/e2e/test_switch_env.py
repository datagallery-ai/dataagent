...
async def main():
    ...
    agent1 = DataAgent.from_config(config_path)
    agent2 = DataAgent.from_config(config_path)
    agent2.build_agent_graph("chat")
    agent2.update_config({"ENV": {"module": "dataagent.actions.environment.Env"}})
    query = "What is 5 + 3 * 2"
    try:
        response = await agent1.chat(query)
