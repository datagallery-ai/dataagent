...
_UNTRUSTED_PROMPT_VALUE_MAX_CHARS = 2000


def _format_untrusted_prompt_value(value: Any) -> str:
    # Action input/output is model-visible data, so mark it as untrusted.
    try:
        text = json.dumps(value, ensure_ascii=False, sort_keys=True, default=str)
    except (TypeError, ValueError):
        text = repr(value)
    if len(text) > _UNTRUSTED_PROMPT_VALUE_MAX_CHARS:
        text = text[:_UNTRUSTED_PROMPT_VALUE_MAX_CHARS] + "...[truncated]"
...
def build_past_action(context: Context) -> str:
    ...
    for idx, action_label in enumerate(active_nodes):
        ...
        safe_input = _format_untrusted_prompt_value(ir.params)
        safe_output = _format_untrusted_prompt_value(ir.output)
        header = (
            f"[IR Summary {idx:02d}] tool={ir.action}, input={safe_input}, success={ir.success}, output={safe_output}"
        )
