...
def _serialize(msg: BaseMessage) -> dict[str, Any]:
    ...
    # Idempotently stamp a creation timestamp (epoch seconds) onto the message
    # object's additional_kwargs. This persists across re-serializations (the
    # same message object may be serialized many times by repeated save_messages
    # full-overwrite calls), so _compute_round_summaries can derive per-round
    # elapsed time from the first→last timestamp within each round.
...
def _compute_round_summaries(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def _extract_ts(rec: dict[str, Any]) -> float | None:
        ...
        ts = akw.get("_ts") if isinstance(akw, dict) else None
    ...
    for rec in records:
        ...
        if t == "HumanMessage":
            if round_usage is not None and round_has_ai:
                ...
                round_idx += 1
                # Reset accumulator for the new round (fixes cross-round token
                # accumulation bug where the previous round's tokens leaked
                # into the next round's summary).
            ...
            if round_usage is None:
                ...
                # Consecutive HumanMessage within the same round — update end_ts.
