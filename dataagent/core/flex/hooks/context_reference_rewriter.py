...
- Stage C：改写 LLM 绑定指代目标 + 代码确定性展开 + 写回。
...
_PATH_FILTERED_NODE_TYPES = frozenset({"Table", "File", "Script"})
...
2. 若存在指代且能从候选列表中**唯一**确定目标，输出 decision="rewrite" 并给出 resolved_refs。
...
- decision="rewrite" 时 resolved_refs 非空，每项含 mention、target_node、reason。
- mention 必须是用户原始问题中的**连续子串**，与原文完全一致（系统将据此自动展开节点内容）。
- rewrite_query 可由系统根据 resolved_refs 自动生成，可填占位或与原文相同。
...
def _build_candidate_entry(trajectory: Any, node_id: str, attrs: dict[str, Any]) -> dict[str, Any]:
    ...
    if node_type in {"Table", "File"}:
        ...
        script_path = _truncate_text(attrs.get("path"))
        if script_path:
        ...
        entry["summary"] = _truncate_text(
            attrs.get("content") or attrs.get("reasoning_content") or attrs.get("current_status")
        )

    return entry


def _format_node_expansion(candidate_entry: dict[str, Any]) -> str:
    """将候选节点格式化为可写入 query 的确定性展开片段（不含内部 node_id）。

    Args:
        candidate_entry: ``_build_candidate_entry`` 产出的候选字典。

    Returns:
        面向 Planner 的自然语言描述，不暴露 ``Type(label)`` 内部标识。
    """
    node_type = str(candidate_entry.get("node_type") or "").strip()
    description = _truncate_text(candidate_entry.get("description"))

    if node_type == "Table":
        path = _truncate_text(candidate_entry.get("path"))
        if path:
            return f"表（路径 {path}）"
        if description:
            return f"表（{description}）"
        return "表"
    ...
    if node_type == "Script":
        parts = ["脚本"]
        script_type = _truncate_text(candidate_entry.get("script_type"))
        if script_type:
            parts.append(script_type)
        script_path = _truncate_text(candidate_entry.get("path"))
        if script_path:
            parts.append(f"路径 {script_path}")
        summary = _truncate_text(candidate_entry.get("summary"))
        if summary:
            parts.append(summary)
        return "，".join(parts)

    if node_type == "Action":
        action_name = _truncate_text(candidate_entry.get("action_name"))
        params_summary = _truncate_text(candidate_entry.get("params_summary"))
        success = candidate_entry.get("success")
        parts = ["操作"]
        if action_name:
            parts.append(f"工具 {action_name}")
        if params_summary:
            parts.append(f"参数 {params_summary}")
        if success is not None:
            parts.append("成功" if bool(success) else "失败")
        return "，".join(parts)

    if node_type == "Tool":
        parts = ["工具结果"]
        params_summary = _truncate_text(candidate_entry.get("params_summary"))
        returns_summary = _truncate_text(candidate_entry.get("returns_summary"))
        if params_summary:
            parts.append(f"参数 {params_summary}")
        if returns_summary:
            parts.append(f"返回 {returns_summary}")
        return "，".join(parts)
    ...
    if node_type == "State":
        summary = _truncate_text(candidate_entry.get("summary"))
        if summary:
            return f"分析状态：「{summary}」"
        return "分析状态"

    if description:
        return f"{node_type}，{description}" if node_type else description
    return node_type or "上下文对象"


def _apply_reference_expansions(
    raw_query: str,
    resolved_refs: list[dict[str, Any]],
    candidates_by_id: dict[str, dict[str, Any]],
) -> tuple[str | None, str]:
    """在原始 query 中将指代表达替换为确定性节点展开片段。

    Args:
        raw_query: 用户原始问题。
        resolved_refs: 校验通过的指代绑定列表。
        candidates_by_id: ``node_id`` → 候选条目的映射。

    Returns:
        (expanded_query, skip_reason)：成功时 skip_reason 为空；失败时 expanded_query 为 None。
    """
    if not raw_query.strip():
        return None, "empty_raw_query"
    if not resolved_refs:
        return None, "empty_resolved_refs"

    sorted_refs = sorted(
        resolved_refs,
        key=lambda item: len(str(item.get("mention") or "")),
        reverse=True,
    )
    ...
    for ref in sorted_refs:
        ...
        if mention not in expanded_query:
            return None, f"mention_not_in_raw_query:{mention}"
        candidate = candidates_by_id.get(target_node)
        if candidate is None:
            return None, f"candidate_missing:{target_node}"
        expansion = _format_node_expansion(candidate)
        expanded_query = expanded_query.replace(mention, expansion, 1)

    if expanded_query.strip() == raw_query.strip():
        return None, "expansion_unchanged"
    ...
    *,
    ...
    for node_id, attrs in trajectory.nodes(data=True):
        ...
        if node_type not in allowed_types:
            continue
        if node_type in _PATH_FILTERED_NODE_TYPES:
            node_path = str(attrs.get("path") or "").strip()
            if node_path and is_framework_internal_artifact_path(node_path):
                continue
...
def _prepare_reference_rewrite_candidates(
    context: Context,
    raw_query: str,
    analysis: QueryAnalysis,
) -> tuple[list[dict[str, Any]] | None, str]:
    """Stage B：按分析结果收集指代改写候选。

    Args:
        context: 当前会话 Context。
        raw_query: 用户原始 query。
        analysis: Stage A 查询意图分析结果。

    Returns:
        ``(candidates, temporal_hint)``；无候选时 candidates 为 None 并已记录 skip。
    """
    ...
    return candidates, temporal_hint


def _resolve_rewrite_plan_from_candidates(
    runtime: Runtime,
    raw_query: str,
    candidates: list[dict[str, Any]],
    *,
    ...
    """Stage C（前半）：调用改写 LLM 并校验指代绑定计划。

    Args:
        runtime: Flex 运行时。
        raw_query: 用户原始 query。
        candidates: Stage B 候选列表。
        temporal_hint: 时间语义提示。

    Returns:
        校验通过的改写计划；失败时记录 skip 并返回 None。
    """
...
def _commit_resolved_reference_rewrite(
    state: FlexState,
    context: Context,
    raw_query: str,
    plan: RewritePlan,
    ...
    """Stage C（后半）：代码展开指代并写回 state 与 QueryNode。
    ...
    """
    candidates_by_id = {str(candidate["node_id"]): candidate for candidate in candidates}
    expanded_query, expand_skip = _apply_reference_expansions(
        raw_query,
        plan.resolved_refs,
        candidates_by_id,
    )
    if expanded_query is None:
        _log_outcome(
            ...
            skip_reason=expand_skip,
        )
        return state

    if plan.rewrite_query and plan.rewrite_query != expanded_query:
        logger.debug(
            "[context_reference_rewriter] llm rewrite_query ignored: llm={} expanded={}",
            plan.rewrite_query,
            expanded_query,
        )

    original_user_query = str(state.get("user_query") or raw_query)
    state["user_query"] = expanded_query
    if not _sync_query_node(context, expanded_query):
...
def _apply_context_reference_rewrite(
    ...
    candidates, temporal_hint = _prepare_reference_rewrite_candidates(context, raw_query, analysis)
    if candidates is None:
        return state

    plan = _resolve_rewrite_plan_from_candidates(
        runtime,
        raw_query,
        candidates,
        temporal_hint=temporal_hint,
    ...
    return _commit_resolved_reference_rewrite(state, context, raw_query, plan, candidates)
