...
`OUTPUT_DIR/step2_5_wide_userfiltered.csv`。部署 SQL 由下游 `step4_1` 生成。
...
- **唯一 tabular 输入**：`OUTPUT_DIR/step2_5_wide_userfiltered.csv`。不得读取
...
| `step2_5_wide_userfiltered.csv` | 唯一训练数据 |
...
| 类型 | 位置 |
|------|------|
| 唯一训练输入宽表 | `OUTPUT_DIR/step2_5_wide_userfiltered.csv` |
...
Bash 强制预检 `step2_5_wide_userfiltered.csv` 存在且非空；执行前后校验固定 Python 文件集合与
SHA-256。局部重跑仅改 `STEP_START` / `STEP_END`。

## step3_1：分层切分训练/验证集

执行 `step3_1_y_label_split.py`，读取 `step2_5_wide_userfiltered.csv`。
