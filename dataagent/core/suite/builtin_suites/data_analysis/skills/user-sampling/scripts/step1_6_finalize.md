...
| 前置条件 | 来源 | 检查要点 |
|---|---|---|
| `step1_0_sampling_plan.json` | step1_0（step1_1/step1_2追加） | `inventory_check.ok == true`；cold_start时 `similar_games` 或 fallback 已写入 |
| `step1_output_meta.json` | step1_5 | `table_count_check.ok == true`，`missing_tables` 为空，字段符合 `step1_5_stats.md` §5 |
| 全部 `step1_sampled_*` 表 | step1_4 | 实库中张数 = `inventory_check.table_count`；gate 通过 |
...
- step1_1…step1_5 按序完成；step1_5 自查无未修异常
...
- `step1_output_meta.table_count_check.ok == true`；用户表 = 源列 + `label`
