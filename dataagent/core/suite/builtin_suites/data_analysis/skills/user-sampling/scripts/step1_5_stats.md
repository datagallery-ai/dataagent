...
```sql
CREATE OR REPLACE TABLE <database>.step1_temp_stats_user_label
ENGINE = MergeTree()
ORDER BY tuple()
AS
SELECT
...
FROM <database>.step1_sampled_<user_table>;
```
...
```sql
CREATE OR REPLACE TABLE <database>.step1_temp_stats_rows_<table>
ENGINE = MergeTree()
ORDER BY tuple()
AS
SELECT count() AS rows
FROM <database>.step1_sampled_<table>;
```
