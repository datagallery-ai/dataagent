-- step2_4: 动态特征聚合与衍生（ClickHouse）
-- 输入: step2_3_wide_cleaned + step2_1 分类出的全部 1:N / 时序 / 游戏维表
-- 输出: {{database}}.step2_4_wide_complete
-- 动态块按 schema_resolution 与画像结果生成，不能假定固定表数、字段名或业务取值。
-- 本文件整体必须作为一条独立 ClickHouse MCP command 提交。

CREATE OR REPLACE TABLE {{database}}.step2_4_wide_complete
ENGINE = MergeTree
ORDER BY <user_id>
AS
WITH
...
FROM {{database}}.step2_3_wide_cleaned AS w
