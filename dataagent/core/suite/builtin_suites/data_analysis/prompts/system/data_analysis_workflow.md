...
- Use `advance_data_analysis_workflow(...)` only for explicit state transitions:
  ...
  - `action="submit_current_step"` submits a ready current step to its configured subagent. Leave `task` empty to use the persisted step target; change a failed step target through the control tool rather than using a one-off task override. Omit `workspace_rel_path` to allocate a fresh subagent workspace.
  ...
  - `action="retry_current_step"` only resets a failed current step to ready; pass `retry_reason`, then call `submit_current_step` again. To reuse the failed step's previous subagent workspace, first call `search_workspaces` (and optionally `inspect_workspace`) to obtain `workspace_rel_path` (for example `subagents/{id}`), then pass that path into `submit_current_step`. Omit `workspace_rel_path` only when a fresh workspace is preferred.
- Do not call `submit_subagent(...)` directly while a workflow is active. Workspace reuse must go through `advance_data_analysis_workflow(action="submit_current_step", workspace_rel_path=...)`.
- Before submitting a ready step, review its persisted target. If it does not give the subagent all business information needed to complete that step, call `control_data_analysis_workflow(action="update_step_target", ...)` first. Keep the revised target concise and business-level; do not copy the conversation or prescribe implementation steps. Do not use the one-off `task` argument to supply missing context.
- If completion returns `current_step_failed`, the job may have failed or the receipt/artifact acceptance may have failed. Optionally update that failed step target when the recovery requires a changed objective; otherwise retry the current step with `retry_current_step`, discover the prior workspace via `search_workspaces` / `inspect_workspace` when reuse is wanted, and resubmit with `submit_current_step`. Do not invent receipt content.
