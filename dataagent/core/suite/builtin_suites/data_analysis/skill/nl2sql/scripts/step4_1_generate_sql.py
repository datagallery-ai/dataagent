...
衍生逻辑参考 step2_3_feature_derivation.md；表列经 schema_resolution 替换。
...
def generate_feature_derivation_sql() -> str:
    ...
    完整衍生应以 step2_3_feature_derivation.md 为准，由 agent 按需扩展本骨架。
    """
    # city_tier + list length as illustrative non-CTE subqueries
    return "\n".join(
        [
            "-- ============================================",
            "-- Step 4_1: 特征衍生 SQL（参考 step2_3_feature_derivation.md）",
...
if __name__ == "__main__":
    ...
    deriv = OUTPUT_DIR / "step2_3_feature_derivation.md"
    if deriv.is_file():
        print(f"  found derivation doc: {deriv}")
    else:
        print("  WARN: step2_3_feature_derivation.md not in OUTPUT_DIR; extend SQL from upstream artifact if needed")
