...
内部添加 `LIMIT`，并按 SQL 中实际出现的物理关系次数动态分配总读取预算；默认单个关系最多
读取 128 行，名义总读取量不超过 `max_rows_to_read` 的四分之一，同时设置小 block、时间、
线程、读取字节和内存上限。达到读取行数上限时允许 ClickHouse 返回部分试算结果，避免把资源
保护误判为 SQL 错误；禁止只在 final SQL 外层添加 LIMIT。该试算用于验证真实源库中的表、
字段、函数、类型、别名和 JOIN。
