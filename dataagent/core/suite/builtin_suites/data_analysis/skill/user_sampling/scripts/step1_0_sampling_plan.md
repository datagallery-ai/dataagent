...
<必须>description 保真规则：这是最容易出错的环节，请逐字遵守：</必须>

| 规则 | 说明 |
|------|------|
| **逐字搬运** | `columns[].description` <必须>原样取自 `semantic_retrieve` 返回的 `dataAccessPlan.tables[].columns[].description`</必须>，不得改写、摘要、截断 |
| **括号/枚举全保留** | 语义返回若含括号内的枚举值、编码映射、补充说明等，**必须原样完整写入**，禁止只保留括号前的短标题 |
| **排序/TopN说明保留** | 语义返回若含 `TOPN`、`按XX排序`、`截断` 等说明文字，**全量保留** |
| **语义无描述则留 null** | 语义侧某列 `description` 为 `null` 时，**不得自行编造**，写 `null` |

<禁止>
- **禁止**去掉括号内的枚举/映射/补充说明
- **禁止**缩写为短标签（把完整的描述截断成前缀）
- **禁止**概括语义明确列出的值列表
- **禁止**在语义有描述时自行改写、重新措辞
</禁止>
