...
Pipeline **step1（采样）**：<必须>源表在 `source_database` 中（只读），产出投影表写入 `output_database`，表名与源表同名</必须>；行数按采样用户削减，保留源表列集；产出的用户表额外含 **`label`** 列（事件口径追加 / prelabeled 保留源列）。
...
- **交付表**<必须>表名与源表同名，写于 `output_database`，一张都不能少</必须>；（`inventory_check.table_count`，与 `source_table_inventory.tables` 一致）；行数按采样用户削减；保留源表列集；用户表含 `label`（event_derived 追加 / prelabeled 保留源列）。
...
| 类型 | 命名 | 说明 |
|---|---|---|
| 表结构文件 | `step1_0_table_schema.json` | 语义查库结果，sampler 内部拼 SQL 用 |
| 采样计划 | `step1_0_sampling_plan.json` | step1_0 创建，sampler 内部各步读取 |
| 交付投影表 | `<源表名>`（output_database 内，与源表同名） | 与源表一对一，行数缩到采样用户，用户表含 `label` |
