...
# Fixed TTL for main-agent session workspace locks (product decision: 30 minutes).
MAIN_SESSION_LOCK_TTL_SECONDS = 30 * 60
...
class WorkspaceBusyError(RuntimeError):
    ...
    def from_workspace(cls, workspace_root: Path | str) -> WorkspaceBusyError:
        ...
        lock_dir = root / ".lock"
        meta = _read_lock_payload(lock_dir / "lock.json")
        cleanup_guidance = (
            "\n锁将在过期后自动清理。若需手动清理，请先确认占锁的 agent 已停止，"
            f"再删除锁目录：{lock_dir}。占锁 agent 仍在运行时删除锁可能导致多个 agent 并发写入 workspace。"
        )
        if not meta:
            return cls(f"Session workspace is busy: {root}{cleanup_guidance}")
        ...
        return cls(
            ...
            f"purpose={purpose}, expires_at={expires_at}){cleanup_guidance}"
        )
