...
class StorageConnectorGaussVector(BaseStorageConnector):
    ...
    def insert_data(self, table_name: str, data: dict) -> None:
        """
        Insert a row into tables in GaussVector.
    ...
    def delete_data(
        ...
        """
        Delete rows satisfying specific conditions from tables in GaussVector.
    ...
    def execute_sql_and_fetch_dict(
        ...
        """
        Execute SQL query and fetch results as a list of dictionaries.

        Args:
            cursor: Database cursor object with execute, fetchall, and description attributes.
            sql (str): SQL query to be executed.

        Returns:
            List[Dict[str, Any]]: Query results represented as a list of dictionaries.

        Raises:
            ValueError: If no column metadata is available after query execution.
        """
        self.gs.execute(sql)
    ...
    def query_fulltext(self, table_name: str, query_schema: str, query_text: str, topk: int) -> list[dict]:
        """
        Full text search in GaussVector.
    ...
    def query_exact(
        ...
        """
        Exact value search in GaussVector.
    ...
    def query_metadata_column_vector(
        ...
        """
        str_vec = f"[{','.join(map(str, query_vector))}]"
        query_str = f"SELECT * FROM {table_name} WHERE type = '{query_type}' ORDER BY {query_schema} <-> \
            '{str_vec}' LIMIT {topk}"
        return self.execute_sql_and_fetch_dict(query_str)
    ...
    def query_metadata_column_fulltext(
        ...
        Returns:
            List[dict], list of column information.
        """
        query_str = f"SELECT * FROM {table_name} WHERE type = '{query_type}' AND {query_schema} LIKE \
            '%{query_text}%' LIMIT {topk}"
        return self.execute_sql_and_fetch_dict(query_str)

    def query_all(self, table_name: str) -> list[dict]:
        """
        Get full table from GaussVector.
    ...
    def query_relationship(self, table_name: str, exists_field: str) -> list[dict]:
        ...
        Returns:
            List[dict], list of row contents.
        """
        query_str = f"SELECT * FROM {table_name} WHERE {exists_field} IS NOT NULL"
        self.gs.execute(query_str)
        if self.gs.description is not None:
            cur_columns = [desc[0] for desc in self.gs.description]
        else:
...
class LogConnectorPostgreSQL(BaseLogConnector):
    """
    Connect to PostgreSQL to read and write logs.
    """

    def __init__(self, url: str) -> None:
        """
        Connect to PostgreSQL and ensure necessary tables exist.
        If the specified database does not exist, it will be created.
        If the required tables do not exist, they will be created.

        Args:
            url (str): The PostgreSQL connection URL.
        """
        try:
        ...
        try:
            with admin_engine.connect().execution_options(isolation_level="AUTOCOMMIT") as admin_conn:
                ...
                if not exists:
                    logger.debug(f"Database '{dbname}' does not exist. Creating...")
                    admin_conn.exec_driver_sql(f'CREATE DATABASE "{dbname}"')
                    logger.debug(f"Database '{dbname}' created.")
        except Exception as e:
            logger.error(f"Error while checking/creating database: {e}")
        finally:
            admin_engine.dispose()

        # 之后用连接到目标数据库的engine去建表以及进行后续操作
        engine = create_engine(url_obj)  # url_obj包含目标数据库名
        # 测试连接以确保数据库可用
        try:
            with engine.connect() as conn_test:
                conn_test.execute(text("SELECT 1"))
        except Exception as e:
            raise ConnectionError(f"Cannot connect to target database '{dbname}'. ") from e

        # 将 target_engine 赋给 engine 继续后续创建表等操作
        table_list = ["raw_log", "trimmed_log", "min_log"]
        for table in table_list:
            ...
            try:
                ...
                raise RuntimeError(f"Error creating table '{table}'.") from e

        self.sql_reader = PostgreSQLReader(url=url)
        self.engine = engine
        self.type2table = {"raw": "raw_log", "trimmed": "trimmed_log", "min": "min_log"}

    def build_read_log_sql(
        self,
        table: str,
        memory_prefix: str,
        session_id: str | None,
        ...
        """
        Build SQL query string for reading logs.

        Args:
            table (str): Target table name.
            memory_prefix (str): Memory prefix.
            session_id (Optional[str]): Session ID filter.
            start_time (Optional[Timestamp]): Start time filter.
            end_time (Optional[Timestamp]): End time filter.
            limit (int): Row limit.

        Returns:
            str: SQL query string.
        """
        prefix = f"history.{memory_prefix}".replace("'", "''")

        where_conditions = [f"prefix = '{prefix}'"]

        if start_time:
            where_conditions.append(f"created_at >= '{start_time}'")
        if end_time:
            where_conditions.append(f"created_at <= '{end_time}'")

        if session_id:
            key_esc = session_id.replace("'", "''")
            where_conditions.append(f"\"key\" = '{key_esc}'")

        where_clause = " AND ".join(where_conditions)

        return f'SELECT * FROM "{table}" WHERE {where_clause} ORDER BY created_at DESC LIMIT {limit}'
    ...
    def read_log(
        ...
        """
        Read logs from PostgreSQL based on session_id and log_type.
        """
        table = self.type2table.get(log_type)
        if table is None:
            raise ValueError(f"Unknown log type: {log_type}")

        if not isinstance(limit, int) or limit <= 0:
            raise ValueError("limit must be a positive integer")
        if not isinstance(memory_prefix, str) or not memory_prefix:
            raise ValueError("memory_prefix must be a non-empty string")
        if start_time is not None and not isinstance(start_time, Timestamp):
            raise ValueError("start_time must be a pandas Timestamp object")
        if end_time is not None and not isinstance(end_time, Timestamp):
            raise ValueError("end_time must be a pandas Timestamp object")
        if start_time and end_time and start_time >= end_time:
            raise ValueError("start_time must be earlier than end_time")

        sql_str = self.build_read_log_sql(
            table=table,
        ...
        try:
            res = self.sql_reader.execute_sql(text(sql_str))
        except Exception as e:
            raise RuntimeError("Failed to execute SQL using PostgreSQLReader, trying raw connection") from e
        ...
        for record in res.to_dict(orient="records"):
            if isinstance(record, str):
                try:
                    ...
                    logger.error(f"parse json failed. {e}")
            else:
    ...
    def save_log(
        ...
        Args:
            ...
            expires_at (Optional[Timestamp]): The expiration timestamp of the log (Default: `None`).
        """
        if not memory_prefix or not isinstance(memory_prefix, str):
            raise ValueError("memory_prefix must be a non-empty string")
        ...
        if isinstance(value, dict):
            ...
            raise ValueError("Value must be a dict representing JSON data")

        insert_sql = f"""
        INSERT INTO {table} (prefix, "key", value, created_at, updated_at, ttl_minutes, expires_at)
        VALUES (:prefix, :key, CAST(:value AS jsonb), :created_at, :updated_at, :ttl_minutes, :expires_at)
        """
        try:
            with self.engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
                conn.execute(
                    text(insert_sql),
                    {
                        "prefix": f"history.{memory_prefix}",
                        "key": session_id.replace("'", "''"),
                        "value": value_json,
            ...
            logger.error(f"Error saving log to table '{table}'. Error: {e}")

    def delete_log(self, memory_prefix: str, session_id: str, log_type: str | None = None) -> None:
        """
        ...
        Args:
            ...
            log_type (str, optional): The type of log to delete ('raw', 'trimmed', 'min'). If `None`, delete from
                all log types (Default: `None`).
        """
        if log_type and log_type not in self.type2table:
            raise ValueError(f"Unknown log type: {log_type}")

        if log_type is None:
            for lt in self.type2table:
                self.delete_log(memory_prefix, session_id, lt)
            return

        delete_sql = f"""
        DELETE FROM {self.type2table.get(log_type)}
        WHERE prefix = :prefix AND "key" = :key
        """
        try:
            with self.engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
                conn.execute(text(delete_sql), {"prefix": f"history.{memory_prefix}", "key": session_id})
            logger.debug(
                f"Deleted logs from table '{log_type}' for prefix '{memory_prefix}' and session_id '{session_id}'"
            )
        except Exception as e:
            raise RuntimeError(f"Error deleting log from table '{log_type}'.") from e
...
class PostgreSQLReader:
    """
    Connector for PostgreSQL database tables.
    """

    def __init__(self, url: str) -> None:
        """
        Initialize connection to target PostgreSQL database.

        Args:
            url (str): PostgreSQL url to target database.
        """
        self.engine = create_engine(url)

    def load_table(self, table_name: str) -> pd.DataFrame:
        """
        Load table in PostgreSQL to pandas dataframe.
        ...
        if "." in table_name:
            ...
            table_name_exact = f'"{temp[0]}"."{temp[1]}"'
        else:
            table_name_exact = f'"{table_name}"'

        sql_command = f"select * from {table_name_exact}"
        df = pd.read_sql(sql_command, con=self.engine)
        return df

    def execute_sql(self, sql_command: str | TextClause) -> pd.DataFrame:
        """
        Execute a SQL command and return the result as a pandas DataFrame.
        ...
        """
        return pd.read_sql(sql_command, con=self.engine)
