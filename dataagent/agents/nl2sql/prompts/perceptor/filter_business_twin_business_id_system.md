...
- crh_group | 维度 | 高铁分群
- crh_railway_id | 维度 | 高铁线路
- crh_section_id | 维度 | 高铁线路段
- crh_station_id | 维度 | 高铁站点
...
- accuracy_of_public_usr_id_of_nwdaf | 指标 | 公网用户识别准确率
- accuracy_of_rail_usr_id_of_nwdaf | 指标 | 高铁用户识别准确率
...
- att_create_rail_usr_char_ana_pcf_of_nwdaf | 指标 | 接收PCF发送的RAIL_USR_CHAR_ANALYSIS事件订阅创建请求次数
- att_create_sub_session_amf_of_nwdaf | 指标 | 向AMF发送事件订阅请求次数
- att_public_usr_events_notify_pcf_of_nwdaf | 指标 | 向PCF发送订阅结果通知请求次数-用户属性为公网用户
- att_rail_usr_events_notify_pcf_of_nwdaf | 指标 | 向PCF发送订阅结果通知请求次数-用户属性为高铁用户
- att_update_rail_usr_char_ana_pcf_of_nwdaf | 指标 | 接收PCF发送的RAIL_USR_CHAR_ANALYSIS事件订阅更新请求次数
- att_update_sub_session_amf_of_nwdaf | 指标 | 向AMF发送事件订阅更新请求次数
- avg_qoe | 指标 | MOS分数总和
- avg_rail_usr_char_ana_event_of_nwdaf | 指标 | NWDAF上RAIL_USR_CHAR_ANALYSIS事件平均在线订阅事件数
- bit_rate | 指标 | 码率总和
...
- crh_sub_users | 指标 | 高铁签约用户数
- crh_trigger_first_n23_create_times | 指标 | 首次N23会话创建上报次数
- crh_trigger_nwdaf_n23_cancel_subs_times | 指标 | NWDAF触发的N23取消订阅次数
- crh_trigger_pcf_n23_cancel_subs_times | 指标 | PCF触发的N23取消订阅次数
- crh_trigger_pra_in_times | 指标 | PRA_IN次数
- crh_trigger_pra_out_times | 指标 | PRA_OUT次数
- crh_trigger_usertype_chg_times | 指标 | 用户类型切换次数
- crh_unsub_users | 指标 | 高铁未签约用户数
- crh_users | 指标 | 高铁画像用户数
- cur_act_public_usr_num_of_nwdaf | 指标 | 当前在线的公网用户数
- cur_de_num_of_public_usr_of_nwdaf | 指标 | 当前在线的公网用户在专网数
- de_acc_num_of_public_usr_of_nwdaf | 指标 | 公网用户接入高铁专网次数（空闲态进入连接态）
- de_acc_num_of_rail_usr_of_nwdaf | 指标 | 高铁用户接入高铁专网次数（空闲态进入连接态）
- de_mig_num_of_public_usr_of_nwdaf | 指标 | 公网用户迁出高铁专网次数（空闲态进入连接态）
- de_mig_num_of_rail_usr_of_nwdaf | 指标 | 高铁用户迁出高铁专网次数（空闲态进入连接态）
- delay_an | 指标 | 无线时延总和
...
- downlink_avg_duration | 指标 | 下行平均总时长
- downlink_avg_traffic | 指标 | 下行平均总流量
...
- max_act_public_usr_num_of_nwdaf | 指标 | 最大在线的公网用户数
- max_act_rail_usr_num_of_nwdaf | 指标 | 最大在线的高铁用户数
...
- max_rail_usr_char_ana_event_of_nwdaf | 指标 | NWDAF上RAIL_USR_CHAR_ANALYSIS事件最大在线订阅事件数
- max_resolution*_times | 指标 | 最高分辨率的次数分布
- med_duration_of_public_usr_id_of_nwdaf | 指标 | 公网用户识别时长（中位数）
- med_duration_of_rail_usr_id_of_nwdaf | 指标 | 高铁用户识别时长（中位数）
...
- poor_mos_times | 指标 | MOS质差次数
- poor_service_delay_times | 指标 | 业务时延质差次数
- poor_stalling_duration_times | 指标 | 卡顿时长质差次数
- poor_total_times | 指标 | 总质差次数
- pub_users | 指标 | 公网用户数
- record_count | 指标 | 单据数
- record_duration_100_150 | 指标 | 单据时长100-150ms计数
- record_duration_150_200 | 指标 | 单据时长150-200ms计数
- record_duration_200_250 | 指标 | 单据时长200-250ms计数
- record_duration_50_100 | 指标 | 单据时长50-100ms计数
- record_duration_gt_250 | 指标 | 单据时长>250ms计数
- record_duration_lt_50 | 指标 | 单据时长<50ms计数
...
- stalling_duration_times | 指标 | 卡顿时长计数
- stalling_number | 指标 | 卡顿次数总和
- sub_app_eff_duration | 指标 | 应用有效时长
- succ_create_rail_usr_char_ana_pcf_of_nwdaf | 指标 | 向PCF回复RAIL_USR_CHAR_ANALYSIS事件订阅创建成功次数
- succ_create_sub_session_amf_of_nwdaf | 指标 | 接收AMF事件订阅成功次数
- succ_public_usr_events_notify_pcf_of_nwdaf | 指标 | 接收PCF回复的订阅结果通知成功次数-用户属性为公网用户
- succ_rail_usr_events_notify_rail_usr_pcf_of_nwdaf | 指标 | 接收PCF回复的订阅结果通知成功次数-用户属性为高铁用户
- succ_update_rail_usr_char_ana_pcf_of_nwdaf | 指标 | 向PCF回复RAIL_USR_CHAR_ANALYSIS事件订阅更新成功次数
- succ_update_sub_session_amf_of_nwdaf | 指标 | 接收AMF事件订阅更新成功次数
...
- unk_users | 指标 | 未知用户数
- uplink_avg_duration | 指标 | 上行平均总时长
- uplink_avg_traffic | 指标 | 上行平均总流量
