...
@pytest.mark.asyncio
async def test_reflector_stops_loop_on_repeated_sql_but_keeps_sql():
    ...
    with patch("dataagent.agents.nl2sql.nodes.reflector.logger.warning") as warn:
        out = await ReflectorNode._aprocess(node, state)
...
@pytest.mark.asyncio
async def test_reflector_exhausted_retries_still_proceeds_with_best_sql():
...
@pytest.mark.asyncio
async def test_reflector_proceeds_when_score_ok():
    """Reflector should proceed when the best candidate meets its threshold."""
    node = _node()
...
@pytest.mark.asyncio
async def test_reflector_does_not_record_seen_sql_before_proceed():
    """Fingerprint must not enter seen_sqls until this round adopts a passing SQL."""
    node = _node()
    node._fix_sql = AsyncMock(return_value=["SELECT fixed"])
...
@pytest.mark.asyncio
async def test_reflector_fix_path_clears_residual_error():
    """Reflector should clear stale execution errors after producing fixed SQL."""
    node = _node()
    node._fix_sql = AsyncMock(return_value=["SELECT fixed"])
