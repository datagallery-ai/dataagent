...
# ---------------------------------------------------------------------------
# Issue #8: Plan-Mandatory 三层防御 — L2/L3 单测
# ---------------------------------------------------------------------------


def _fake_context_no_plan():
    """无 plan 的伪 Context，仅暴露 _build_plan_prompt_variables 访问的属性。"""
    return SimpleNamespace(todolist_manager=SimpleNamespace(todolist=None))


def _fake_context_with_plan():
    """有 plan 的伪 Context；todos 含 1 已完成 + 1 未完成。"""
    plan = SimpleNamespace(
        introduction="Create neutralization experiment",
        approach="Query samples then insert experiment",
        todos=[
            SimpleNamespace(title="Query antibody and virus samples", completed=True),
            SimpleNamespace(title="Insert experiment record", completed=False),
        ],
    )
    return SimpleNamespace(todolist_manager=SimpleNamespace(todolist=plan))


def _ai_with_read_file_skill_md():
    """构造一条调用 read_file(skill/foo/SKILL.md) 的 AIMessage。"""
    return AIMessage(
        content="",
        tool_calls=[
            {
                "name": "read_file",
                "args": {"path": "skill/create-neutralization-experiment/SKILL.md"},
                "id": "tc-skill-md-1",
            }
        ],
    )
...
def test_plan_prompt_variables_counts_tool_messages_from_state():
    """state['messages'] 含 3 条 ToolMessage → tool_call_count == 3。"""
    state = {
        "messages": [
    ...
    vars = planner_prompt_builder._build_plan_prompt_variables(_fake_context_no_plan(), state=state)
    assert vars["tool_call_count"] == 3
    assert vars["has_plan"] is False


def test_plan_prompt_variables_detects_skill_md_read():
    """state 含 read_file(skill/foo/SKILL.md) 的 AIMessage → skill_md_read_without_plan == True。"""
    state = {
        "messages": [_ai_with_read_file_skill_md(), ToolMessage(content="md content", tool_call_id="tc-skill-md-1")]
    }
    vars = planner_prompt_builder._build_plan_prompt_variables(_fake_context_no_plan(), state=state)
    assert vars["skill_md_read_without_plan"] is True
    assert vars["has_plan"] is False


def test_plan_prompt_variables_skill_md_flag_false_when_plan_exists():
    """有 plan 时即使读过 SKILL.md，skill_md_read_without_plan == False。"""
    state = {"messages": [_ai_with_read_file_skill_md()]}
    vars = planner_prompt_builder._build_plan_prompt_variables(_fake_context_with_plan(), state=state)
    assert vars["has_plan"] is True
    assert vars["skill_md_read_without_plan"] is False
    assert vars["plan_current_todo"] == "Insert experiment record"


def test_todo_template_escalates_when_tool_count_exceeds_threshold(monkeypatch):
    """tool_call_count=5 + has_plan=False → todo 段含 PLAN REQUIRED。"""
    monkeypatch.setenv("DATAAGENT_PLAN_REQUIRED_TOOL_THRESHOLD", "4")
    tpl = PromptTemplate.from_package_relative(f"{PROMPT_MD_PREFIX}/planner/todo")
    rendered = build_human_message(
        tpl,
        prompt_str="",
        has_plan=False,
        ...
        plan_required_threshold=4,
    ).content
    assert "PLAN REQUIRED" in rendered
    assert "5 tool call(s)" in rendered
    assert "create_plan" in rendered


def test_todo_template_no_escalation_when_plan_exists():
    """has_plan=True → todo 段不含 PLAN REQUIRED，含 Current todo。"""
    tpl = PromptTemplate.from_package_relative(f"{PROMPT_MD_PREFIX}/planner/todo")
    rendered = build_human_message(
        tpl,
        prompt_str="",
        has_plan=True,
        plan_all_todos_done=False,
        plan_introduction="Create exp",
        plan_approach="approach",
        plan_current_todo="Insert experiment record",
        plan_todos_overview="- [x] Query samples\n- [ ] Insert experiment record",
        tool_call_count=10,
        skill_md_read_without_plan=False,
        plan_required_threshold=4,
    ).content
    assert "PLAN REQUIRED" not in rendered
    assert "Current todo" in rendered
    assert "Insert experiment record" in rendered


def test_user_template_renders_mandatory_constraint_wording():
    """planning_instructions 非空 → user 段含 MANDATORY，不含 'only when relevant'。"""
    tpl = PromptTemplate.from_package_relative(f"{PROMPT_MD_PREFIX}/planner/user")
    rendered = build_human_message(
        tpl,
        user_query="创建中和实验",
        database_context="",
        planning_instructions="必须先调用 create_plan",
        working_directory="/tmp/ws",
        allow_path_lines="",
        memory="",
    ).content
    assert "MANDATORY" in rendered
    assert "only when they are relevant" not in rendered
    assert "必须先调用 create_plan" in rendered


def test_prepare_flex_planner_prompt_injects_skill_md_warning(monkeypatch, tmp_path):
    """state 含 SKILL.md read 且无 plan → messages 含 [SYSTEM POLICY] 注入。"""
    from dataagent.core.context.context import ContextFactory
    from dataagent.core.flex.utils.planner_prompt_builder import prepare_flex_planner_prompt
    ...
    context = ContextFactory.get_context(user_id="u", session_id="s", run_id=1, sub_id=0)
    context.register_query("创建BD55-1111和XBB.1.5的中和实验", [])
    ...
    state = {
        ...
        "run_id": 1,
        "sub_id": 0,
        "user_query": "创建BD55-1111和XBB.1.5的中和实验",
        "messages": [_ai_with_read_file_skill_md()],
        "planner_user_sync_pending": True,
    }
    messages = prepare_flex_planner_prompt(
        context,
    ...
    assert any("[SYSTEM POLICY]" in c for c in contents), (
        "expected [SYSTEM POLICY] injection when SKILL.md read without plan"
    )
    assert any("create_plan" in c for c in contents)
